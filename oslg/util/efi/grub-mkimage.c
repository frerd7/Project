	     ...
	     grub_uint32_t size)
{
  ...
  o->code_base = sections[1].virtual_address;
...
static void
generate_image (const char *dir, char *prefix, FILE *out,
		char *memdisk_path, char *config_path)
{
  ...
  modinfo = grub_obj_add_modinfo (obj, dir, 1,
...
\n\
  ...
  -m, --memdisk=FILE      FILE as a memdisk image [default=DIR/boot.disk]\n\
...
main (int argc, char *argv[])
{
  ...
  generate_image (dir ? : GRUB_LIBDIR, prefix ? : DEFAULT_DIRECTORY, fp, memdisk, config);
