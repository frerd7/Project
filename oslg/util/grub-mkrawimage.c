...
static void
generate_image (const char *dir, char *prefix, FILE *out, char *config_path)
{
  ...
  modinfo = grub_obj_add_modinfo (obj, dir, 1,
				  0, config_path);
#else
  modinfo = grub_obj_add_modinfo (obj, dir, 0,
				  0, config_path);
#endif
...
main (int argc, char *argv[])
{
  ...
  while (1)
    {
      int c = getopt_long (argc, argv, "d:p:c:o:hVv", options, 0);
  ...
  generate_image (dir ? : GRUB_LIBDIR, prefix ? : DEFAULT_DIRECTORY, fp, config);
