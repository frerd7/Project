...
grub_obj_add_modinfo (struct grub_util_obj *obj, const char *dir, int as_info,
		      char *memdisk_path __attribute__ ((unused)), char *config_path)
{
  __attribute__ ((unused)) size_t memdisk_size = 0;
 __attribute__ ((unused)) char *kernel_path, *info, *biosdisk_path, *ext2, *fshelp_path;
  int offset, total_module_size, config_size;
  struct grub_util_obj_segment *seg;
  char *part_msdos, *part_gpt;
  ...
  part_msdos = grub_util_get_path (dir, "part_msdos.mod");
  info = add_module (obj, part_msdos, info, &offset);
  part_gpt = grub_util_get_path (dir, "part_gpt.mod");
  info = add_module (obj, part_gpt, info, &offset);

  fshelp_path = grub_util_get_path (dir, "fshelp.mod");
  info = add_module (obj, fshelp_path, info, &offset);
  ext2 = grub_util_get_path (dir, "ext2.mod"); 
  info = add_module (obj, ext2, info, &offset);
  ...
  free(ext2);
    ...
    char *search_fs_uuid_path;
    biosdisk_path = grub_util_get_path (dir, "biosdisk.mod");
    info = add_module (obj, biosdisk_path, info, &offset); 
    search_fs_uuid_path = grub_util_get_path (dir, "search_fs_uuid.mod");
    info = add_module (obj, search_fs_uuid_path, info, &offset);

    free(biosdisk_path);
    free(search_fs_uuid_path);
#endif

  total_module_size = offset;
#if defined(GRUB_TARGET_X86_64)
  if (memdisk_path)
    {
      ...
      mem_dir = grub_util_get_path (dir, "boot.disk");
      memdisk_size = ALIGN_UP(grub_util_get_image_size (mem_dir), 512);
      grub_util_info ("the size of memory disk is 0x%x", memdisk_size);
      total_module_size += memdisk_size + sizeof (struct grub_module_header);
      }
#endif
  ...
  if (memdisk_path)
    {
    ...
    else
     {
      ...
      grub_util_load_image (mem_dir, info + offset);
      offset += memdisk_size;
     }
#endif
